...
1. Replace placeholders
   - **Recommended: Use GitHub Actions workflow**
     - Go to **Actions** → **Instantiate Template** → **Run workflow**
     - Fill in the form with your project details (name, owner, emails, etc.)
     - The workflow will automatically replace all placeholders and commit changes
...
2. Update repository settings
   - Set repository-level **Actions variables**
     (Settings → Secrets and variables → Actions → Variables) used by workflows.
...
3. Check policy files
   - Edit `CITATION.cff`, `LICENSE`, `CODE_OF_CONDUCT.md`, and `SECURITY.md`
     to include project-specific contact details and owners.
...
- If DCO fails: amend the latest commit with `git commit --amend -s --no-edit`
  then push. For multiple historic commits, use interactive rebase
  (`git rebase -i <commit>`) and add sign-offs as needed,
